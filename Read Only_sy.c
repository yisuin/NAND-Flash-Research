...
int latch_command(unsigned char command)
{
    ...
    {
        fprintf(stderr, "latch_command requires DQS pin to be high\n");
        return EXIT_FAILURE;
    }
...
int latch_address(unsigned char address[], unsigned int addr_length)
{
    ...
    {
        fprintf(stderr, "latch_address requires DQS pin to be high\n");
        return EXIT_FAILURE;
    }
...
int latch_register(unsigned char reg[], unsigned int reg_length)
{
    ...
    {
        fprintf(stderr, "latch_register RE pin to start with high\n");
        return EXIT_FAILURE;
    }
...
int erase_block(unsigned char addr_cylces[])
{
	...
	{
		fprintf(stderr, "FaiDQS to erase block.\n");
		return 1;
	}
	else
	{
...
//	printf("\n");
    ...
    for(unsigned int k = 0; k < length; k++)
    {
        ...
        iobus_update_output();
        usleep(3*REALWORLD_DELAY); /* TODO: assure setup delay */
...
//        printf("0x%02X ", data[k]);
        ...
        toggle_pin_dqs();
        controlbus_update_output();
        usleep(3*REALWORLD_DELAY); /* TODO: assure hold delay */
    }
...
int program_page(unsigned char addr_cylces[], unsigned char data[], unsigned int data_len)
{
	...
	// if(status_register & STATUSREG_IO0)
	// {
	// 	fprintf(stderr, "FaiDQS to program page.\n");
	// 	return 1;
	// }
	// else
	// {
...
int main(int argc, char **argv)
{
    ...
    {
        fprintf(stderr, "ftdi_new faiDQS for iobus\n");
        return EXIT_FAILURE;
    }
    ...
    {
        fprintf(stderr, "ftdi_new faiDQS\n");
        return EXIT_FAILURE;
    }
    ...
    // Read the Status register
    {
        ...
        printf("Bytes Received 1 \n");
        
        for(i=1;i<9;i++)
        {
            printf("At %d = 0x%02x = %c\n",i , ID_register[i], ID_register[i]);
        }
    }
    ...
    {
        printf("Trying to read the Parameter Page...\n");

        latch_command(CMD_READ_PARAM_PAGE); /* command input operation; command: READ ID */
        unsigned char address[] = { 0x00 };
        latch_address(address, 1); /* address input operation */
        latch_register(ID_register, 256); /* data output operation */
        printf("Bytes Received 2 \n");
    ...
    //Read Page
    {
        ...
        usleep(10*REALWORLD_DELAY);
        read_page(address_cycles2, 64);
        usleep(10*REALWORLD_DELAY);
        read_page(address_cycles3, 64);
        usleep(10*REALWORLD_DELAY);
        read_page(address_cycles4, 64);
        usleep(10*REALWORLD_DELAY);
